...
class COSObjectDownloader:
    ...
    def download_objects(self, objects, local_dir='downloads', prefix_filter=''):
        ...
        elapsed_time = time.time() - start_time
        print(f"下载完成！成功下载 {downloaded_count} 个对象，总耗时 {elapsed_time:.2f} 秒")
...
def main():
    ...
    try:
        ...
        # 询问是否要筛选特定前缀
        prefix = config['options']['prefix'] if 'options' in config and 'prefix' in config['options'] else ''
        ...
        # 显示对象统计信息
        total_size = sum(int(obj['Size']) for obj in objects if isinstance(obj['Size'], (int, str)))
        print(f"\n对象统计:")
